...
export default function Registerpw ({
    ...
    const effectiveness = () => {
        const judge = strongPassword(password)
        if (!judge) {
    ...
    return(
        <div>
            ...
            />
            <p className= {`font-NanumSquareRound text-red-600 text-[14px]`}>{effectivenessCheck}</p>
            <InputStore
                inputStyle={InputStyle.INPUT_WHITE}
                value={checkpw}
                inputType="password"
            ...
            />
            <p className= {`font-NanumSquareRound ${textColor} text-[14px]`}>{emailCheck}</p>
        </div>
    )
}